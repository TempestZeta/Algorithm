...
public class Bandage {
    public int solution(int[] bandage, int health, int[][] attacks) {
        int answer = health;
        int time  = 0;
        int bonusHealTick = bandage[0];
        int healPerTick = bandage[1];
        int bonusHeal = bandage[2];

        for(int index = 0; index < attacks.length; index++) {

            int termOfAttack = attacks[index][0];
            int healEnableTime = termOfAttack - 1 - time;

            int bonusTimes = healEnableTime / bonusHealTick;
            int heal = (healEnableTime * healPerTick) + (bonusTimes * bonusHeal);
            int healResult = answer + heal;

            answer = Math.min(health, healResult);
